...
assert((app.match(/'\/[a-z][^']*':'[a-z]/g)||[]).length===22,'SPA route registry must contain 22 product views');

const css=fs.readFileSync(path.join(project,'assets/css/tokens.css'),'utf8');
['#12140F','#F5F2E7','#8CA05E','#5B6F3B','max-width:1000px','max-width:720px','max-width:420px'].forEach(token=>assert(css.includes(token),`Missing design token/breakpoint: ${token}`));
...
console.log('Life Planner SPA smoke test passed: one entry point, 22 hash routes, local assets, responsive tokens, storage, and backup/restore.');
