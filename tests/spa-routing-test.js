...
(async()=>{
  ...
  const dom=new JSDOM(html,{runScripts:'dangerously',pretendToBeVisual:true,url:'https://life-planner.local/index.html#/financial-planner/annual-dashboard'}),w=dom.window,d=w.document;
  assert.equal(d.querySelector('#page-title').textContent,'See where this month stands');
  const firstChart=w.__charts[0];assert(firstChart,'Finance chart did not initialize');
  w.location.hash='/task-tracker/variable-tasks';await new Promise(resolve=>w.setTimeout(resolve,0));
  assert.equal(d.querySelector('#page-title').textContent,'Plan the work that moves life forward');assert(firstChart.destroyed,'Old chart was not destroyed on route change');
  ...
  const values=new Map(),fileDom=new JSDOM(html,{runScripts:'dangerously',pretendToBeVisual:true,url:'file:///C:/Life-Planner/index.html#/financial-planner/annual-dashboard',beforeParse(window){Object.defineProperty(window,'localStorage',{value:{getItem:key=>values.has(key)?values.get(key):null,setItem:(key,value)=>values.set(key,String(value))}})}}),fw=fileDom.window;
